...
const createUrlParam = (json) => {
  ...
  keys = keys.sort();
  let _arr = keys.map(key => `${key}=${json[key]}`);
  return _arr.join('&');
};
...
const sendMoney = (data, callback) => {
  ...
  let wechatReq = https.request(opt, res => {
  ...
  let _sendData = getRedPacketPostXml(data);
  logger.info('-------send-------------');
