    ...
    const calculateWrongWords = (openId, date1, date2, user, isLookDetail) => {
      ...
      if (totalTaskAmount > 0) {
        ...
        return user.save().then(user => {
          return {touser: openId, money: todayMoney1, xxTaskAmount, totalTaskAmount, xxWrongTaskAmount, date1: date1.toLocaleString(), date2: date2.toLocaleString(), reward: reward1, rightTaskRate: rightTaskRate};
        }).catch(e => {
...
const sendModelMessage = (data, accessToken) => {
  ...
  const totalMoney = parseFloat(data.money) + parseFloat(data.reward);
  try {
  request.post({
