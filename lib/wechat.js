...
const queryTodayUserMoney = (date1, date2) => {
  ...
  return query.find().then(results => {
    ...
    return Promise.all(promiseArray).then(() => {
      console.log('shouldSendMoney:' + JSON.stringify(shouldSendMoney));
...
const getUserTaskData = (openId, date1, date2, user) => {
  ...
  return Promise.all([totalTaskAmountPromise, xxTasksPromise]).then(results => {
    ...
    if (rightTaskRate >= 0.7 && rightTaskRate < 0.8) {
      rewardRate += 0.05;
    } else if (rightTaskRate >= 0.8 && rightTaskRate < 0.9) {
      rewardRate += 0.07;
    } else if (rightTaskRate >= 0.9 && rightTaskRate <= 1) {
      rewardRate += 0.2;
    } else {
      rewardRate = 0;
    }
    ...
    const reward =  todayMoney1 * rewardRate;

    user.set('wrong_words_rate', wrongWordsRateList);
    user.set('reward_rate', rewardRate);
    user.set('salary', todayMoney1);
    user.set('right_task_rate', rightTaskRate);
    user.set('total_task_amount', totalTaskAmountLC + totalTaskAmount);
    user.set('task_amount', totalTaskAmount);// 昨天任务数

    console.log('userJson:' + JSON.stringify(user));

    if (!user) {
      // console.log('errorTask:' + JSON.stringify(errorTask));
      return {errorTask, reward, rewardRate};
    }
    if (totalTaskAmount > 0) {
      return user.save().then(user => {
        return {touser: openId, money: todayMoney1, xxTaskAmount, totalTaskAmount, xxWrongTaskAmount, date1: date1.toLocaleString(), date2: date2.toLocaleString(), reward};
      });
    } else {
      return new Promise(() => {
        return {totalTaskAmount: 0};
      });
    }
  });
};
...
const sendModelMessage = (data, accessToken) => {
  ...
  request.post({
    ...
    body:  {
      ...
      data: {
        first: {
          value: `细节至关重要，它值得被耐心等待。\n 恭喜你，今天的正确率是${data.rightTaskRate}，正确率奖励${data.reward}元！`,
          color: '#173177'
        },
        keynote1: {
          value: '任务反馈',
        ...
        remark: {
          value: `正确片段的薪酬：${data.money}元 \n 正确率奖励：${data.reward}元 \n 总收入：${data.money + reward} \n\n！！！请注意！！！
                  \n 1）如果连续3天正确率过低，将会被取消“领取任务”功能！
                  \n 2）如果正确率大于90%，当日将多获得20%的薪酬奖励，每日叠加，直到100%！\n 点击查看详情`,
