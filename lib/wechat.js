...
const getUserTaskData = (openId, date1, date2, user) => {
  ...
  return Promise.all([totalTaskAmountPromise, xxTasksPromise]).then(results => {
    const calculateWrongWords = (openId, date1, date2, user, isLookDetail) => {
      ...
      const wrongWordsRate = (xxWrongWordsAmount / xxWordsAmount).toFixed(2),
            wrongTaskRate = (xxWrongTaskAmount / xxTaskAmount).toFixed(2),
      ...
      if (isLookDetail) {
        console.log('errorTask:' + JSON.stringify({errorTask, reward, rewardRate}));
      ...
      if (totalTaskAmount > 0) {
        ...
        return user.save().then(user => {
          return {touser: openId, money: todayMoney1, xxTaskAmount, totalTaskAmount, xxWrongTaskAmount, date1: date1.toLocaleString(), date2: date2.toLocaleString(), reward: reward, rightTaskRate: rightTaskRate};
        });
      } else {
